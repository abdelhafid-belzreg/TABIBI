...
import {
  LayoutDashboard, Calendar, User, Search, LogOut, Menu, X,
} from "lucide-react";
      ...
      >
        ...
        {open && (
          <div className="p-3 border-bottom text-center">
            <div
              className="rounded-circle bg-success bg-opacity-10 d-flex align-items-center justify-content-center mx-auto mb-2"
              style={{ width: 48, height: 48 }}
            >
              <User size={22} className="text-success" />
            </div>
            <div className="fw-semibold small text-truncate">{user?.name}</div>
            <div className="text-secondary" style={{ fontSize: "0.7rem" }}>Patient</div>
          </div>
        )}
